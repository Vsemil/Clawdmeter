...
static void resolve_act_lists(void);   // activity tiers, defined with ACT_NAMES
...
static void resolve_group_lists(void) {
    ...
    // Same resolution for the activity tiers (ACT_NAMES, declared below).
    resolve_act_lists();
}
...
// Activity tiers: the busier the host, the livelier Clawd gets. Same shape as
// GROUP_NAMES (literal names resolved against splash_anims[] at init), just
// keyed by working-session count instead of usage rate. The last tier is the
// dance floor — played at double tempo.
#define ACT_TIER_COUNT 5
#define ACT_MAX        4
...
static uint8_t act_rotation[ACT_TIER_COUNT] = {0};

static void resolve_act_lists(void) {
    for (int t = 0; t < ACT_TIER_COUNT; t++) {
        act_size[t] = 0;
        for (int s = 0; s < ACT_MAX; s++) {
            act_lists[t][s] = -1;
            const char* want = ACT_NAMES[t][s];
            if (!want) continue;
            for (int i = 0; i < SPLASH_ANIM_COUNT; i++) {
                if (strcmp(splash_anims[i].name, want) == 0) {
                    act_lists[t][act_size[t]++] = (int8_t)i;
                    break;
                }
            }
        }
...
static int activity_tier(void) {   // -1 unknown, else index into ACT_NAMES
...
// Switch the main splash canvas to animation `idx` and restart its playback.
...
// The rotation the current state calls for: activity when the daemon reports
// it, usage rate otherwise.
static void pick_next_anim(void) {
    if (activity_state >= 0) pick_for_activity();
    else                     splash_pick_for_current_rate();
}

// Frame hold, halved on the dance floor.
static uint16_t frame_hold(const splash_anim_def_t *a) {
    uint16_t hold = a->holds[cur_frame];
    return (activity_tier() == ACT_TIER_DISCO) ? (uint16_t)(hold / 2) : hold;
}
...
void splash_tick(void) {
    ...
    if (now - last_pick_ms >= SPLASH_ROTATE_INTERVAL_MS) {
        if (walk_active) {
            if (walk_phase == 0 && pb_done) pick_next_anim();
        } else {
            loop_release = true;
            pending_pick = true;
    ...
    if (now - frame_started_ms < frame_hold(a)) return;
...
static void pick_for_activity(void) {
    ...
    if (tier < 0 || act_size[tier] == 0) { splash_pick_for_current_rate(); return; }
    uint8_t n = act_size[tier];
    uint8_t pick = (uint8_t)act_lists[tier][act_rotation[tier]++ % n];
    if (pick == cur_anim && n > 1)      // avoid an immediate repeat
        pick = (uint8_t)act_lists[tier][act_rotation[tier]++ % n];
    if (pick == cur_anim) {             // single-track tier already playing —
        last_pick_ms = millis();        // let it run instead of restarting it
        return;
    }
    start_anim(pick);
}
...
void splash_show(void) {
    pick_next_anim();                 // select animation; direct path defers the draw
